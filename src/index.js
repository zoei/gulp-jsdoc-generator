...
let debug = require('debug')('gulp-jsdoc-generator');
    ...
    return map(function (file, callback) {
        ...
        new Promise(function (resolve, reject) {
            ...
            fs.writeFile(tmpobj.name, JSON.stringify(jsdocConfig), 'utf8', function (err) {
                ...
                } else if ((jsdocConfig.opts && jsdocConfig.opts.template)) {
                    let templatePath
                    try {
                        templatePath = path.dirname(require.resolve(jsdocConfig.opts.template));
                    } catch (e) {
                        templatePath = path.resolve(jsdocConfig.opts.template)
                    }
